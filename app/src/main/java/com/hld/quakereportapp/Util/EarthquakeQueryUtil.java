package com.hld.quakereportapp.Util;
...
public class EarthquakeQueryUtil {
     ...
     * static final means the value will never change
     */
    ...
    /**
     * Create a private constructor because no one should ever create a {@link EarthquakeQueryUtil} object.
     * This class is only meant to hold static variables and methods, which can be accessed
     * directly from the class name EarthquakeQueryUtil (and an object instance of EarthquakeQueryUtil is not needed).
     */
    private EarthquakeQueryUtil() {
    }
    ...
    public static ArrayList<EarthquakeModel> extractEarthquakes() {
        ...
        //1st step, create the root JSON object by calling the constructor and passing in the sample
        //JSON response string
        //store object in a variable "baseJsonResponse"
        try {
            // TODO: Parse the response given by the SAMPLE_JSON_RESPONSE string and
            // build up a list of Earthquake objects with the corresponding data.
            JSONObject baseJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

           /* get jsonarray with key "feature" that contains a list of earthquake.
            so, call the getjsonarray method on the basejsonresponse object then passing in the
            "feature" key as a string, then store result on variable earthquakeArray*/
            JSONArray eartquakeArray = baseJsonResponse.getJSONArray("feature");

            /* loop throuh each eartquake in the eartquakearray */
            for (int i = 0; i < eartquakeArray.length(); i++){
                //pull out the json object at the specific position of the eartquakearray
                ...
                //create a new eartquake object from these 3 string
                EarthquakeModel earthquakeModel = new EarthquakeModel(magnitude, location, time);

                //add new eartquake to the list of eartquake
                earthquakes.add(earthquakeModel);
            }

        } catch (JSONException e) {
            Log.e("EarthquakeQueryUtil", "problem parsing the earthquake JSON result", e);
        }
